...
public class PainelBatalha extends JPanel{
	...
	private PainelTentativaFuga painelTentativaFuga;
	private PainelHeroiFugiu painelHeroiFugiu;
	...
	private LabelExibePersonagensBatalha labelExibePersonagensBatalha;
 	...
 	public PainelBatalha(){
		...
		painelHeroiFugiu = new PainelHeroiFugiu();
		criaImagemPainel();
		defineRumoBatalha();
		setSize(995, 689);
		setLocation(0, 0);
	...
	private void atualizaPainelSuperior(boolean heroiGanhouJokenpo) {
		if (heroiGanhouJokenpo) {
			...
			painelHeroiGanhouJokenpo.adicionaAcaoTentarFugir(new MouseListener() {
				...
				@Override
				public void mouseEntered(MouseEvent e) {
					painelHeroiGanhouJokenpo.setExibeEscolha("Tentar fugir");
				}
	...
	private void criaPainelTentativaFuga() {
		painelTentativaFuga = new PainelTentativaFuga();
		add(painelTentativaFuga);
		atualizaPainel();
		for (int i = 0; i < 7; i++) {
			painelTentativaFuga.adicionaAcaoPorta(i, criaMouseListenerPorta(i));
		}
		
	}
	
	private MouseListener criaMouseListenerPorta(int i) {
		return new MouseListener() {
			...
			@Override
			public void mouseClicked(MouseEvent e) {
				Controle.getControle().tentaFugir(i);
				if (Controle.getControle().heroiFugiu()) {
					criaLabelHeroiFugiu();
				}else {
					criaLabelHeroiNaoFugiu();
				}
			}
	...
	private void criaLabelHeroiNaoFugiu() {
		painelTentativaFuga.setVisible(false);
		remove(painelTentativaFuga);
		defineRumoBatalha();
		atualizaPainel();
		Controle.getControle().atualizaEstatisticasBatalha();
	}
	
	private void criaLabelHeroiFugiu() {
		painelTentativaFuga.setVisible(false);
		remove(painelTentativaFuga);
		add(painelHeroiFugiu);	
		atualizaPainel();
	}

	public void adicionaAcaoBotaoOkFimBatalha(ActionListener acao){
		painelHeroiFugiu.adicionaAcaoBotaoOk(acao);
